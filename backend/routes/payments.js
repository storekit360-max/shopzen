...
router.post('/payzy/init', requireAuth, paymentInitLimiter, async (req,res) => {
  ...
  try {
    ...
    data.signed_field_names=PAYZY_FIELDS.join(','); data.signature=payzySignature(data,gw.config.secretKey); await require('../models/Order').findByIdAndUpdate(order._id,{paymentMetadata:data}); const payzyBaseUrl=gw.isLive?'https://api.payzypay.lk':'https://api.payzypay.xyz'; const response=await fetch(`${payzyBaseUrl}/checkout/custom-checkout`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(data)}); const raw=await response.text(); let json={}; try{json=JSON.parse(raw)}catch{} const checkoutUrl=(typeof json==='string'?json:'')||json?.data?.url||json?.data?.redirect_url||json?.data?.checkout_url||json?.data?.data?.url||json?.response?.data?.url||json?.url||json?.redirect_url||json?.checkout_url||(raw.match(/https?:\/\/[^\s"'<>]+/i)||[])[0]; if(!response.ok||!checkoutUrl) { console.error('[Payzy init] Unexpected response',response.status,raw.slice(0,1000)); await rollbackOrder(); return res.status(502).json({message:'Payzy did not provide a checkout URL. The order was cancelled; verify the Shop ID, API key, mode, and Payzy custom-checkout activation.'}); } res.json({url:checkoutUrl});
  } catch(e){console.error('[Payzy init]',e.message); try { await rollbackOrder(); } catch (rollbackError) { console.error('[Payzy rollback]', rollbackError.message); } res.status(502).json({message:'Could not start Payzy checkout. The order was cancelled; please try again.'});}
});
